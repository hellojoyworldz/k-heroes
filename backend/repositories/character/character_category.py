...
from models.character.character_category import (
...
class CharacterCategoryDuplicateError(Exception):
    def __init__(self, title: str):
        self.title = title
        super().__init__(f"이미 사용 중인 카테고리명입니다. ({title})")


class CharacterCategoryNotFoundError(Exception):
    def __init__(self, category_id: int):
        self.category_id = category_id
        super().__init__(f"인물 카테고리를 찾을 수 없습니다. (ID: {category_id})")
...
def list_categories(
    ...
    *,
    ...
    page: int = 1,
    page_size: int = 20,
) -> Tuple[List[CharacterCategory], int]:
    conditions = [CharacterCategory.deleted_at.is_(None)]
    if is_active is not None:
        conditions.append(CharacterCategory.is_active.is_(is_active))

    total = db.scalar(
        select(func.count(CharacterCategory.id)).where(*conditions)
    ) or 0
    query = (

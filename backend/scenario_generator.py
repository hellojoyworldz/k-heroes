...
* [중요] 스탯 설계 및 영속성 규칙:
  ...
  - 인물의 고유성을 담아야 합니다.
  - 이 고유 스탯 3가지의 명칭은 향후 전개될 모든 시나리오와 턴의 선택지 결과 연산에서 '토씨 하나 틀리지 않고 동일한 키값'으로 사용될 것이므로, 매우 신중하고 명확하게 명명하십시오.
...
5. 스탯(stats) 키값의 절대 동기화 규칙 (★최우선 순위 사안):
   - 선택지 A와 B의 'stats' 내부에 들어갈 3가지 키값은 반드시 앞서 인물 카드 데이터에서 정의된 고유 스탯 3가지의 명칭("{stats_keys}")과 '글자 한 자, 공백 하나' 틀리지 않고 100% 일치해야 합니다.
   - 프롬프트에 임의로 정의된 '국력', '백성의 지지', '성공 확률' 등 범용 키값은 예시일 뿐이므로 이를 절대로 출력하지 마십시오. 오직 해당 인물의 고유 스탯 키 3개만 사용하여 수치를 연산하십시오.
